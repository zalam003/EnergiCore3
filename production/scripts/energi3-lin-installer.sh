...
echo
echo "This script will setup or transform an already running node to run Energi 3.x"
echo "Scripts will be provides with the installation to run a masternode and/or staking"
echo "node on a 24/7 basis with minimal intervention. If you have an existing Energi 3"
echo "keystore account file or a dumpwallet file from Energi 2.x, you will need to"
echo "transfer the file to the VPS."
REPLY=''
...
_setup_two_factor() {
  ...
  then
    ...
    wget -4qo- ${GITURL}/otp.php -O "${HOME}/___otp.php"
  fi
