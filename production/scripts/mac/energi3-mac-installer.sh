...
_install_energi3 () {
  ...
  ${SUDO} wget -4qo- "${BIN_URL}" -O "${ENERGI3_EXE}" --show-progress --progress=bar:force 2>&1
  sleep 0.3
  ...
  wget -4qo- "${SCRIPT_URL}/scripts/${NODE_SCRIPT}?dl=1" -O "${NODE_SCRIPT}" --show-progress --progress=bar:force 2>&1
  sleep 0.3
  ...
  wget -4qo- "${SCRIPT_URL}/scripts/${MN_SCRIPT}?dl=1" -O "${MN_SCRIPT}" --show-progress --progress=bar:force 2>&1
  sleep 0.3
  chmod 755 ${MN_SCRIPT}
  ...
  wget -4qo- "${SCRIPT_URL}/js/${JS_SCRIPT}?dl=1" -O "${JS_SCRIPT}" --show-progress --progress=bar:force 2>&1
  sleep 0.3
  chmod 644 ${JS_SCRIPT}
...
_setup_two_factor() {
  ...
  then
    ...
    cd ${ETC_DIR}
    wget -4qo- ${SCRIPT_URL}/thirdparty/otp.php -O "otp.php" --show-progress --progress=bar:force 2>&1
    cp "${ETC_DIR}/otp.php" "${USRHOME}/___otp.php"
    ${SUDO} chmod 644 "${USRHOME}/___otp.php"  
